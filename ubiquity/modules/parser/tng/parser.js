...
Parser.prototype = {
  ...
  initialCache: function() {
    ...
    this.patternCache.verbMatcher = matchString(
      [name for (name in allNames(this._verbList,this.lang))]);

    this.patternCache.verbInitialTest = new RegExp(
      '^\\s*('+this.patternCache.verbMatcher+')'+
        (this.usespaces?'(\\s+.*$|$)':'(.*$)'));
    this.patternCache.verbFinalTest = new RegExp(
      (this.usespaces?'(^.*\\s+|^)':'(^.*)')+
        '('+this.patternCache.verbMatcher+')\\s*$');

    this.patternCache.anaphora = new RegExp(
      (this.usespaces?'(^.*\\s+|^)':'(^.*)')+
        '('+(this.anaphora.join('|'))+')'+
        (this.usespaces?'(\\s+.*$|$)':'(.*$)'));
    ...
    for (let verb in this._verbList) {
      this.rolesCache[verb] =
        [role for each (role in this.roles)
         if (this._verbList[verb].arguments.some(
               function(arg) arg.role == role.role ) ) ];
      this.patternCache.delimiters[verb] = new RegExp('^('
        +[role.delimiter for each (role in this.rolesCache[verb]) ].join('|')
        +')$');
    }

    this.patternCache.delimiters[null] = new RegExp(
      '^('+[role.delimiter for each (role in this.roles) ].join('|')+')$');

  },

  roles: [{role: 'object', delimiter: ''}],
  // a list of roles and their delimiters
  ...
  argFinder: function(argString,verb) {
    ...
    for each (var delimiterIndices in possibleDelimiterCombinations) {
      ...
      for (let i=0; i<delimiterIndices.length; i++) {
        ...
        var newParses = [];
        // we'll update each copy of theseParses and them put them here for the
        // time being
              ...
              if (this.branching == 'left') {// find args right to left
                ...
                thisParse.args[role].push(
                  { _order: 1 + 2*(delimiterIndices.length - i),
                    input:this.cleanArgument(words.slice(j,jmax + 1).
                                             join(this.joindelimiter)),
                    modifier:words[delimiterIndices[i]]
                  });
                ...
                if (j != jmin) {
                  ...
                  thisParse.args.object.push(
                    { _order: 2*(delimiterIndices.length - i),
                      input:this.cleanArgument(words.slice(jmin,j).
                                               join(this.joindelimiter)),
                      modifier:''
                    });
                ...
                thisParse.args[role].push(
                  { _order: 1 + 2*(i)+2 - 1,
                    input:this.cleanArgument(words.slice(jmin,j + 1).
                                             join(this.joindelimiter)),
                    modifier:words[delimiterIndices[i]] });

                // put the extra words between this delimiter and the next
                // in the direct object array
                if (j != jmax) {
                  ...
                  thisParse.args.object.push(
                    { _order: 1 + 2*(i)+2,
                      input:this.cleanArgument(words.slice(j + 1,jmax + 1).
                                               join(this.joindelimiter)),
                      modifier:''
                    });
  ...
  substitute: function(parse,selection) {
    ...
    for (let role in parse.args) {
      ...
      for (let i in args) {
        let newArg;
        if (newArg = args[i].input.replace(this.patternCache.anaphora,
                                           "$1"+selection+"$3")) {
  ...
  /* suggestArgs returns an array of copies of the given parse.
     there may be multiple returned as different noun suggestions are thrown
     in together.
  */
...
Parser.Query.prototype = {
  ...
  _yieldingParse: function() {
    ...
    for each (parse in this._possibleParses) {
      ...
      if (selection.length &&
          this.parser.patternCache.anaphora.test(this._input)) {
    ...
    for each (parse in this._possibleParses) {
      this._verbedParses = this._verbedParses.concat(
        this.parser.suggestVerb(parse,this._threshold));
      yield true;
    }
