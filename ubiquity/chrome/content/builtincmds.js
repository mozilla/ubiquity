...
CmdUtils.CreateCommand({
  ...
  for (var i=0; i < node.form.elements.length; i++) {
    ...
    if (el == node) {
      formData.push((isURLEncoded) ? escapeNameValuePair(el.name, PLACEHOLDER, true) :
                                     // Don't escape PLACEHOLDER, just append
                                     escapeNameValuePair(el.name, "", false) + PLACEHOLDER);
      continue;
    }
  ...
  if (isURLEncoded)
    ...
    //COPY ENDS
    if(postData){ displayMessage("Sorry.  Support for POST-method forms is yet to be implemented."); return;}
    //var url = makeURI(spec)
    var url = spec;
    ...
    all_code = PrefCommands.getCode();
    all_code += code
    PrefCommands.setCode(all_code)
    ...
    displayMessage("You have created the command: "+name+".  You can edit its source-code with the command-editor command.")
  }
});
//// Dependancies:
//Ability to edit the ubiquity's commands, copied from chrome://ubiquity/content/prefcommands.js
var PrefCommands = {
...
//URI manipulation code was copied from /usr/lib/firefox-3.0.1/components/WebContentConverter.js :
...
//This function was copied from chrome://browser/content/browser.js
